...
class ReportRepairAPIController extends Controller
{
    ...
    public function reportRepairAPI(Request $request){
        ...
        $data['repair_request_date'] = now();
        $data['userId'] = $request->userId;
        $data['repair_request_status'] = 'pending';
       ...
       public function reportRepairHistoryAPI($id=null){
        if($id){
            $repair = DB::table('repair_request')->where('userId',$id)->get();
        }else {
            $repair = DB::table('repair_request')->get();
        }
